...
public class StarlightStabEntity extends AbstractHurtingProjectile implements GeoEntity {
    ...
    public StarlightStabEntity(EntityType<? extends StarlightStabEntity> type, Level world) {
        super(type, world);
        this.textureVariant = random.nextInt(7);
    }

    public StarlightStabEntity(Level world, double x, double y, double z, float Damage) {
        this(ModEntities.STARLIGHT_STAB.get(), world);
        this.setPos(x, y, z);
    ...
    public void tick() {
        ...
        if (slowingDown) {
            ...
            if (slowed.lengthSqr() < 0.2) { // Muy lento -> descartamos
                detonateEntity(level(), this.getX() + 0.2, this.getY() + 0.9, this.getZ(), 1, 0.1f, TerramityModParticleTypes.HOLY_GLINT.get());
            }

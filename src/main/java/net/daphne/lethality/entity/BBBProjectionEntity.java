...
public class BBBProjectionEntity extends AbstractHurtingProjectile implements GeoEntity {
    ...
    public BBBProjectionEntity(EntityType<? extends BBBProjectionEntity> type, Level world) {
        super(type, world);
    }

    public BBBProjectionEntity(Level world, double x, double y, double z, float Damage) {
        this(ModEntities.BBB_PROJECTION.get(), world);
        this.setPos(x, y, z);
    ...
    public void tick() {
            ...
            for (int i = 0; i < (int)particleNum; ++i) {
                ...
                for (LivingEntity target : hitEntities) {
                    ...
                    if (!this.messagedEntities.contains(uuid)) {
                        ...
                        target.addEffect(new MobEffectInstance(ModMobEffects.CRUMBLING.get(), 100, 0), this.getOwner());
        ...
        if (slowingDown) {
            ...
            if (slowed.lengthSqr() < 0.3) { // Muy lento -> descartamos
                detonateEntity(level(), this.getX() + 0.2, this.getY() + 0.9, this.getZ(), 25, 0.1f, ModParticles.CUSTOM_PARTICLE.get());
            }

...
public class BloodScytheEntity extends AbstractHurtingProjectile implements GeoEntity {
    ...
    private static final double seekDistance = 25.0;
    ...
    public BloodScytheEntity(EntityType<? extends BloodScytheEntity> type, Level world) {
        super(type, world);
    }

    public BloodScytheEntity(Level world, double x, double y, double z, float Damage) {
        this(ModEntities.BLOOD_SCYTHE.get(), world);
        this.setPos(x, y, z);
    ...
    public void tick() {
        ...
        if (slowingDown) {
            ...
            if (slowed.lengthSqr() < 0.3) { // Muy lento -> descartamos
                detonateEntity(level(), this.getX() + 0.2, this.getY() + 0.9, this.getZ(), 25, 0.15f, TerramityModParticleTypes.EVIL_GLINT.get());
            }

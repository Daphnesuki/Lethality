...
public class ForbiddenScytheEntity extends AbstractHurtingProjectile implements GeoEntity {
    ...
    public ForbiddenScytheEntity(EntityType<? extends ForbiddenScytheEntity> type, Level world) {
        super(type, world);
    }

    public ForbiddenScytheEntity(Level world, double x, double y, double z, float Damage) {
        this(ModEntities.FORBIDDEN_SCYTHE.get(), world);
        this.setPos(x, y, z);
    ...
    public void tick() {
        ...
        if (slowingDown) {
            ...
            if (slowed.lengthSqr() < 0.3) { // Muy lento -> descartamos
                detonateEntity(level(), this.getX() + 0.2, this.getY() + 0.9, this.getZ(), 35, 0.25f, ModParticles.FORBIDDEN_GLINT.get());
            }
